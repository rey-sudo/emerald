...
def build_document(questions: List[QuestionItem]) -> Document:
    ...
    title = doc.add_heading("Banco de Respuestas", level=1)
...
def add_question_block(
    ...
    # Correcta
    correct_label = OPTION_LABELS[item.correct]

    p_answer = doc.add_paragraph()

    run_answer = p_answer.add_run(
        f"Respuesta correcta: {correct_label}"
    )
    run_answer.bold = True

    # Explicación
    p_explanation = doc.add_paragraph()

    run_explanation = p_explanation.add_run("Explicación: ")
    run_explanation.bold = True

    p_explanation.add_run(item.explanation)
...
def export_questions_to_word(
    input_json: str | Path,
    output_docx: str | Path = "score.docx"
) -> Path:
    """
    Pipeline completo:
